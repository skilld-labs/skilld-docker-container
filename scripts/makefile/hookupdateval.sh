...
if [ "$FIELD_STORAGE_COUNT" -gt "0" ]; then
  ...
  if [ "$UPDATE_COUNT" -gt "0" ]; then
    ...
    printf "hook_update_N() missing for storage changes: A config update has occured which requires a hook_update to be properly deployable on live site."
    exit 1
  fi
else
	echo "No changes in field storage."
	exit 0
fi
